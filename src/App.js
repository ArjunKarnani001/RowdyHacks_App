  ...
  useEffect(() => {
    ...
    params: {
        ...
        if (predictions.length > 0) {
          ...
          // Update: Check if the prediction is different before appending
          if (currentPrediction !== lastPredictionRef.current && lastPredictionRef.current !== "") {
            setAppendedText((prevText) => prevText + currentPrediction); // Append predicted text only if different
          ...
          // Always update the last prediction and reset the consecutive count
          lastPredictionRef.current = currentPrediction;
          consecutiveSamePredictionCountRef.current = 1;
